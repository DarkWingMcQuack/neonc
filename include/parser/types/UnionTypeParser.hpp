...
{
public:
    constexpr auto union_type() -> std::expected<ast::Type, common::error::Error>
    {
        using lexing::TokenTypes;
        using lexing::TextArea;

        auto first_type_res = static_cast<T *>(this)->tuple_type();
        ...
        while(union_type_lexer().pop_next_is(lexing::TokenTypes::BITWISE_OR)) {
            auto next_type = static_cast<T *>(this)->tuple_type();
            if(not next_type) {
...
private:
    constexpr auto union_type_lexer() noexcept -> lexing::Lexer &
    {
